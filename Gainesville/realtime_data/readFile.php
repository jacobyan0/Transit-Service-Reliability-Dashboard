<?php
    ...
    while(!feof($file)) {
        ...
        //echo "L.circleMarker([" . $lat . "," . $lon . "], {radius:8, opacity:0.8, fill:'True', fillOpacity:1, weight:3, fillColor:'#".$rtcolor."', color:'".$color."'}).bindTooltip('" . $rt . "', {permanent:true, direction:'center', className:'mylabel'}).addTo(map);";
    }
    $conn->close();
    fclose($file);
?>